...
class classSampler(Sampler):
    def __init__(self, data_source):
        ...
        self.hash_list = [i for i in range(3000)]
...
def create_loaders(split, loader_configs, num_workers, distributed=False):
    ...
    for di,dt in enumerate(split):
        ...
        if distributed:
            sampler = DistributedSampler(dsets[dt], shuffle=(dt == "train"))
        else:
...
class ActivityNetCaptionsDataset(AbstractDataset):
    def __init__(self, config):
        ...
        self.anns, self.qids, self.vids, self.qid_qclass  = self._load_annotation(ann_path)
    ...
    def __getitem__(self, idx):
        ...
        if "class_id" in self.anns[qid].keys():
            class_id = self.anns[qid]["class_id"]
        else:
            class_id = 0
    ...
    def _preprocessing(self, anns, new_anns, qid, vids, mode):
        ...
        for vid in anns.keys():
            ...
            if mode == "train":
                for ts,q,cid in zip(ann["timestamps"], ann["sentences"], ann["class"]):
                #for ts,q in zip(ann["timestamps"], ann["sentences"]):
                    ...
                    qid_qclass.append((qid, int(cid)))
                    qid += 1
            elif mode == "val" or mode == "phrase":
                for ts,q in zip(ann["timestamps"], ann["sentences"]):
                    new_anns[str(qid)] = {
                        "timestamps": ts,
                        "query": q,
                    ...
                    qid_qclass.append((qid, 0))
                    qid += 1
        vids.extend(list(anns.keys()))
        return new_anns, qid, list(set(vids)), qid_qclass
    ...
    def _load_annotation(self, ann_path):
        ...
        if isinstance(ann_path, list):
            ...
            for ap in ann_path:
                ...
                new_anns, qid, vids, qid_qclass = self._preprocessing(anno, new_anns, qid, vids, "val")
        elif "phrase" in ann_path:
            anno = io_utils.load_json(ann_path)
            new_anns, qid, vids, qid_qclass = self._preprocessing(anno, new_anns, qid, vids, "phrase")
        else:
            # for train annotation
            anno = io_utils.load_json(ann_path)
            new_anns, qid, vids, qid_qclass = self._preprocessing(anno, new_anns, qid, vids, "train")
    ...
    def generate_labels(self, config):
        ...
        if not os.path.exists(self.paths["query_labels"]):
            ...
            train_anns, _, _, _ = self._load_annotation(train_ann_path)
