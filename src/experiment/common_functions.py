...
def get_method(method_type):
    if method_type.startswith("tgn"):
        ...
        raise NotImplementedError(
            "Not supported model type ({})".format(method_type))
    return M
...
def get_loader(D, split=[], loader_configs=[], num_workers=2, distributed=False):
    assert len(split) > 0
    assert len(split) == len(loader_configs)
    return D.create_loaders(split, loader_configs, num_workers, distributed)
...
def test(config, loader, net, epoch, eval_logger=None, mode="Test", if_print=False, distributed=False):
    with torch.no_grad():
        if distributed:
            net_ddp = net
            net = net.module
        ...
        for batch in tqdm(loader, desc="{}".format(mode)):
            ...
            if distributed:
                net_out = net_ddp.forward(net_inps, if_print, epoch)
                loss = net.loss_fn(net_out, gts, count_loss=True)
                outputs = {"loss": loss, "net_output": net_out}
            else:
                outputs = net.forward_only(net_inps, gts, if_print, epoch)  # only forward
        ...
        if epoch > 0 and mode != "Phrase" and net.renew_best_score():
            ckpt_path = os.path.join(
                config["misc"]["result_dir"], "checkpoints")
            with open(os.path.join(ckpt_path, "record.txt"), "a+") as f:
                f.write(str(epoch)+"\n")
