...
class Attention(nn.Module):
    ...
    def compute_att_logits(self, key, feats, feat_masks=None):
        ...
        att_f = net_utils.apply_on_sequence(
            self.feat2att, feats)   # B * A * att_hdim
        # B * att_hdim
        att_k = self.key2att(key)
        att_k = att_k.unsqueeze(1).expand_as(
            att_f)                 # B * A * att_hdim

        # compute attention weights
        # B * A * att_hdim
        dot = torch.tanh(att_f + att_k)
...
class VideoEmbeddingWithPosition(nn.Module):
    def __init__(self, config, prefix=""):
        ...
        super(VideoEmbeddingWithPosition, self).__init__()
        name = prefix if prefix is "" else prefix+"_"
    ...
    def forward(self, seg_feats, seg_masks):
        ...
        if self.use_position:
            ...
            pos = torch.arange(0, seg_masks.size(1)).type_as(
                seg_masks).unsqueeze(0).long()
            pos_emb = self.pos_emb_fn(pos)
            B, nseg, pdim = pos_emb.size()
            pos_feats = (pos_emb.expand(B, nseg, pdim) *
                         seg_masks.unsqueeze(2).float())
            seg_emb += pos_feats
...
class QuerySequenceEncoder(nn.Module):
    ...
    def __init__(self, config, prefix=""):
        ...
        self.emb_odim = config.get(name+"emb_odim", 512)
        self.embedding = nn.Embedding(emb_idim, self.emb_odim)
        self.rnn = bn.get_rnn(config, prefix)  # == LSTM
        
        self.word_encode = config.get(name+"wencode", "onehot")
        
        if self.word_encode == "word2vec":
            path = config.get(name+"w2vpath", "")
            self.word_vectors = KeyedVectors.load_word2vec_format(path, binary=True)
            self.word_vectors['<PAD>'] = np.array([0 for i in range(self.emb_odim)])
            self.word_vectors['a'] = self.word_vectors['an']
            self.word_vectors['to'] = self.word_vectors['for']
            self.word_vectors['of'] = self.word_vectors['for']

    def forward(self, itow, onehot, mask, sentence, out_type="all_hidden"):
        ...
        #wemb = [[self.word_vectors[itow[str(onehot[i, j].item())]] for j in range(shape[1])] for i in range(shape[0])]
        ...
        if self.word_encode == "word2vec":
            for i in range(shape[0]):
                ...
                for j in range(shape[1]):
                    if itow[str(onehot[i, j].item())] in self.word_vectors.key_to_index.keys():
                        emb = self.word_vectors[itow[str(onehot[i, j].item())]]
                    else:
                        emb = np.array([0 for i in range(self.emb_odim)])
                    wemb[i].append(emb)
            
            wemb = torch.Tensor(wemb).to("cuda")
        
        elif self.word_encode == "onehot":
            wemb = self.embedding(onehot)  # [B,L,emb_odim]

        if torch.isnan(wemb).sum() > 0:
            print("wencoder:wemb: ", wemb, "--wencoder:wemb")
            pdb.set_trace()
        ...
        if torch.isnan(w_feats).sum() > 0:
            print("wencoder:w_feats: ", w_feats, "--wencoder:w_feats")
            pdb.set_trace()
    ...
    def load_glove(self, wtoi, glove_path):
        ...
        self.embedding.weight.data = net_utils.to_contiguous(
...
class SequentialQueryAttention(nn.Module):
    ...
    def forward(self, q_feats, w_feats, ori_wfeats, w_mask=None):
        ...
        B = w_feats.size(0)
        #L = w_feats.size(1)
        #words_cnt = w_mask.sum(1)
        prev_se = w_feats.new_zeros(B, self.qdim)
        se_feats, se_attw, oriw_sqan = [], [], []
        ...
        for n in range(self.nse):
            ...
            wsqan_feats = att_w.unsqueeze(2).expand(ori_wfeats.size())*ori_wfeats
            wsqan_feats = wsqan_feats.sum(1)

            prev_se = att_f
            
            '''  
            now_attw = torch.zeros_like(w_mask, dtype=torch.float32)
            for i in range(B):
                for j in range(n*words_cnt[i].item()//3, (n+1)*words_cnt[i].item()//3):
                    now_attw[i, j] = 1.0
            att_w = F.softmax(now_attw, dim=1)
            
            wsqan_feats = att_w.unsqueeze(2).expand(ori_wfeats.size())*ori_wfeats
            wsqan_feats = wsqan_feats.sum(1)
            
            att_f = att_w.unsqueeze(2).expand(w_feats.size())*w_feats
            att_f = att_f.sum(1)
            '''
            
            se_feats.append(att_f)
            se_attw.append(att_w)
            oriw_sqan.append(wsqan_feats)

        return torch.stack(se_feats, dim=1), torch.stack(se_attw, dim=1), torch.stack(oriw_sqan, dim=1)
...
class NonLocalBlock(nn.Module):
    ...
    def forward(self, m_feats, mask):
        ...
        for i in range(self.nheads):
            ...
            m2m = mk_slice @ mq_slice.transpose(1, 2) / \
                ((self.odim // self.nheads) ** 0.5)
            if self.use_local_mask:
                local_mask = mask.new_tensor(self._get_mask(
                    nseg, self.ksize, self.dilation))  # [nseg,nseg]
                m2m = m2m.masked_fill(local_mask.unsqueeze(
                    0).eq(0), -1e9)  # [B,nseg,nseg]
            m2m = m2m.masked_fill(mask.unsqueeze(
                1).eq(0), -1e9)  # [B,nseg,nseg]
            m2m_w = F.softmax(m2m, dim=2)  # [B,nseg,nseg]
            w_list.append(m2m_w)

            # compute relation vector for each segment
            r = m2m_w @ mv_slice if (i == 0) else torch.cat((r,
                                                             m2m_w @ mv_slice), dim=2)
...
class LocalGlobalVideoTextInteractions(nn.Module):
    def __init__(self, config, prefix=""):
        ...
        if self.l_type == "res_block":
            self.local_fn = self._make_modulelist(
                ResBlock1D(config, "lgi_local"), self.nse)
    ...
    def forward(self, seg_feats, seg_masks, se_feats):
        ...
        sa_feats, sattw = self._global_context_modeling(
            ss_feats, se_feats, seg_masks)
    ...
    def _local_context_modeling(self, m_feats, masks):
        ...
        for n in range(self.nse):
            if self.l_type == "res_block":
                l_feats = self.local_fn[n](m_feats[n].transpose(
                    1, 2)).transpose(1, 2)  # [B,nseg,*]
            elif self.l_type == "masked_nl":
                l_feats = m_feats[n]
    ...
    def _global_context_modeling(self, ss_feats, se_feats, seg_masks):
        ...
        glob_feats.insert(0, sa_feats)
        return glob_feats, sattw
...
class AttentionLocRegressor(nn.Module):
    ...
    def forward(self, semantic_aware_seg_feats, original_vfeats, masks):
        loc_ls = []
        att_w_ls = []
        sum_vfeat = []
        for i in range(1+self.nse):
            # perform Eq. (13) and (14)
            summarized_vfeat, att_w = self.tatt(
                semantic_aware_seg_feats[i], masks)

            #print(original_vfeats.shape)
            #print(att_w.shape)
            #print(semantic_aware_seg_feats[i].shape)

            att_vfeats = att_w.unsqueeze(2).expand(original_vfeats.size())*original_vfeats
            att_vfeats = att_vfeats.sum(1)
            sum_vfeat.append(att_vfeats)
            #sum_vfeat.append(summarized_vfeat)

            # perform Eq. (15)
            loc = self.MLP_reg(summarized_vfeat)  # loc = [t^s, t^e]
            loc_ls.append(loc)
            att_w_ls.append(att_w)

        return loc_ls, att_w_ls, sum_vfeat
...
class MultipleCriterions(nn.Module):
    ...
    def __init__(self, names=None, modules=None):
        ...
        self.crits = nn.ModuleList(
            modules) if modules is not None else nn.ModuleList()
        self.name2crit = {}
        if names is not None:
            self.name2crit = {name: self.crits[i]
...
class TGRegressionCriterion(nn.Module):
    ...
    def __init__(self, cfg, prefix=""):
        ...
        self.nse = cfg.get("num_semantic_entity", -1)
    ...
    def forward(self, net_outs, gts):
        ...
        total_loss = self.regloss1(
            loc[:, 0], s_gt) + self.regloss2(loc[:, 1], e_gt)
...
class softLoss(nn.Module):
    ...
    def forward(self, x, y1, y2):
        diff = torch.abs(x-y1)
        cond = diff < 1.
        soft = torch.where(cond, 0.5 * diff ** 2, diff - 0.5)
        return torch.sum(torch.mul(soft, torch.div(torch.abs(x-y1), torch.abs(y2-y1)).pow(2)))

class subTGRLoss(nn.Module):
    def __init__(self, cfg, prefix="grounding"):
        super(subTGRLoss, self).__init__()
        self.name = prefix if prefix is "" else prefix+"_"

        self.regloss1 = softLoss()
        self.regloss2 = softLoss()

        self.nse = cfg.get("num_semantic_entity", -1)
        self.w = cfg.get("subtgr_weight", 1.0)
    ...
    def forward(self, net_outs, gts):
        ...
        for i in range(1, self.nse+1):
            loc = net_outs[self.name+"loc"+str(i)]
            total_loss += self.regloss1(
                loc[:, 0], s_gt, e_gt) + self.regloss2(loc[:, 1], e_gt, s_gt)

        return self.w * total_loss / self.nse
...
class DQALoss(nn.Module):
    ...
    def forward(self, net_outs, gts):
        ...
        # pdb.set_trace()
        P = torch.norm(torch.bmm(attw, attw_T) - I,
                       p="fro", dim=[1, 2], keepdim=True)
...
class TAGLoss(nn.Module):
    ...
    def forward(self, net_outs, gts):
        ...
        for i in range(1, self.nse+1):
            mask = gts[self.name+"tag_att_masks"]  # [B,num_segment]
            w = net_outs[self.name+"tag_attw"+str(i)]  # [B,num_segment]
            mask = torch.max(mask, w)
            ac_loss = (-mask*torch.log(w+1e-8)).sum(1) / mask.sum(1)
            ac_loss = (self.w * ac_loss.mean(0))
            total_ac_loss += ac_loss
        

        return total_ac_loss / (self.nse+1)
        '''
        
        return total_ac_loss        
        
class halfL1Loss_1(nn.Module):
    def __init__(self):
        super().__init__()
        self.lossfunc = nn.SmoothL1Loss()
        
    def forward(self, x, y):
        return self.lossfunc(torch.max(x, y), y)

class halfL1Loss_2(nn.Module):
    def __init__(self):
        super().__init__()
        self.lossfunc = nn.SmoothL1Loss()
        
    def forward(self, x, y):
        return self.lossfunc(torch.min(x, y), y)

class IoPLoss(nn.Module):
    def __init__(self, cfg, prefix="grounding"):
        super(IoPLoss, self).__init__()
        self.name = prefix if prefix is "" else prefix+"_"
        print("iop Loss - ", self.name)

        self.nse = cfg.get("num_semantic_entity", -1)
        self.w = cfg.get("iop_weight", 1.0)
        self.iop_threshold_max = cfg.get("iop_threshold_max", 0.3)
        self.iop_threshold_min = cfg.get("iop_threshold_min", 0.1)

        self.ioploss_1 = halfL1Loss_1()
        self.ioploss_2 = halfL1Loss_2()

    def cul_inter(self, s1, s2, e1, e2):
        start = torch.max(s1, s2)
        end = torch.min(e1, e2)
        inter = end - start
        zero = torch.zeros_like(inter)
        inter = torch.max(inter, zero)
        return inter
    ...
    def forward(self, net_outs, gts):
        ...
        for i in range(1, self.nse+1):
            loc = net_outs[self.name+"loc"+str(i)]  # [B,2]
            
            pd_len = loc[:,1] - loc[:,0]
            inter = self.cul_inter(s_gt, loc[:,0], e_gt, loc[:,0])
            iop = torch.div(inter, pd_len)
            total_loss += self.ioploss_1(iop, torch.full_like(iop, self.iop_threshold_max))
            total_loss += 3*self.ioploss_2(iop, torch.full_like(iop, self.iop_threshold_min))
        
        #print("***********iop_total_loss*****************")
        #print(total_loss)
        return self.w * total_loss


class SOFLoss(nn.Module):
    def __init__(self, cfg, prefix=""):
        super(SOFLoss, self).__init__()
        self.name = prefix if prefix is "" else prefix+"_"

        print("Similarity of Features Loss - ", self.name)
        self.nse = cfg.get("num_semantic_entity", -1)
        self.w = cfg.get("sof_weight", 1.0)
        self.margin = cfg.get("sof_margin", 0.2)
        self.lossf = cfg.get("sof_method", "abs")

    def forward(self, net_outs, gts):
    
        #print("***************sofloss_forward******************")
        
        se_feats = net_outs[self.name+"subq_f"].detach()  # [B*nse*d1]
        sa_feats = net_outs[self.name+"attf"]  # [nse*B*d2]
        total_loss = 0
        
        #print("********************sof_info***************************")
        #print(type(sa_feats))
        #print(sa_feats)
        #sa_feats = torch.Tensor(sa_feats)
        
        if self.nse > 1:
            subq_f = se_feats[:,0,:]
        else:
            subq_f = se_feats.squeeze(1)
        fused_f = sa_feats[0]
        for i in range(1, self.nse):
            subq_f = torch.cat((subq_f, se_feats[:,i,:]), 0)
            fused_f = torch.cat((fused_f, sa_feats[i]), 0)
        
        subq_f = F.normalize(subq_f, p=2, dim=1)
        fused_f = F.normalize(fused_f, p=2, dim=1)
            
        #print(subq_f.shape)
        #print(fused_f.shape)
            
        subq_T = torch.transpose(subq_f, 0, 1).contiguous()
        fused_T = torch.transpose(fused_f, 0, 1).contiguous()
            
        subq_sim = torch.mm(subq_f, subq_T)
        fused_sim = torch.mm(fused_f, fused_T)

        #import pdb
        #pdb.set_trace()
        
        if self.lossf == "abs":    
          total_loss = torch.abs(subq_sim-fused_sim).sum()
        elif self.lossf == "margin":
          total_loss = torch.max(torch.zeros_like(fused_sim), 
                  fused_sim-subq_sim-self.margin)+torch.max(
                  torch.zeros_like(fused_sim), subq_sim-fused_sim-self.margin)
        total_loss = total_loss.mean()
        
        #if type(total_loss) is not torch.Tensor:
            #total_loss = torch.from_numpy(np.array(total_loss)).to("cuda")
            
        #print("***********sof_total_loss*****************")
        #print(total_loss)        
        # import pdb
        # pdb.set_trace()        
        return self.w * total_loss
