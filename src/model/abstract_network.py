...
class AbstractNetwork(nn.Module):
    ...
    @abstractmethod
    def forward(self, net_inps, if_print, epoch):
        """ Forward network
        Args:
    ...
    def loss_fn(self, crit_inp, gts, count_loss=True):
        ...
        for name in self.loss.keys():
            ...
            # for debug
            if torch.isnan(self.loss[name]).sum() > 0:
                print("loss: ", name, self.loss[name], "--loss", name)
                pdb.set_trace()
    ...
    def update(self, loss):
        ...
        total_loss.backward()
        #self.print_grad(loss)
        if self.scheduler is not None:
            self.scheduler.step()
        self.optimizer.step()
        self.optimizer.zero_grad()  # set gradients as zero before updating the network

    def forward_update(self, net_inps, gts, if_print=False, epoch=-1):
    ...
    def forward_only(self, net_inps, gts, if_print=False, epoch=-1):
        """ Compute loss and network's output at once
        Args:
            net_inps: inputs for network; dict()
            gts: ground truth; dict()
        Returns:
            {loss, net_output}: two items of dictionary
                ...
                - net_output: first output from self.forward(); dict()
        """
        net_out = self.forward(net_inps, if_print, epoch)
        loss = self.loss_fn(net_out, gts, count_loss=True)
        return {"loss": loss, "net_output": net_out}
    ...
    def create_optimizer(self):
        ...
        if scheduler_type == "step":
            ...
            def lambda1(it): return it // decay_step
            def lambda2(it): return decay_factor ** it
    ...
    def _get_print_list(self, mode):
        if mode == "Train":
            ...
            print_list = print_list + copy.deepcopy(self.criterion.get_names())
    ...
    def print_counters_info(self, logger, epoch, mode="Train"):
        ...
        if self.use_tf_summary:
            ...
            #for k in val_list:
                #v = self.counters[k]
                #self.writerX.add_scalar(k, v, epoch)
...
#        new_dict = {k: v for k, v in pretrained_dict.items() if k in model_state_dict.keys()}
#        model_state_dict.update(new_dict)
            
        for m in model_state_dict.keys():
            print("---***---***---***---***---")
            print(model_state_dict[m].keys())
            
 #           if m == "vti_fn":
#              pretrained_dict = self[m].state_dict()
#              new_dict = {k: v for k, v in pretrained_dict.items() if k in model_state_dict[m].keys()}
#              model_state_dict[m]=new_dict
#              print("==================================")
#              print(new_dict.keys())
#              print("==================================")
